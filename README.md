...
### 现实的考虑
实际种蓝玫瑰的过程，发现时间比预期的久，而且堆量的话太占地方了。进一步细化策略，决定测交，用另外的花色测试基因。此处以蓝3为基准，这个看上去出率最高。

在第5步骤，只有1个红的基因是需要的，另1个红基因是废的。反推回去，其实在4这一步，2种橙色也是废的，只有1种橙色自交才能出5需要的红，并且这种橙，本身就能小概率产生蓝（蓝2配方）

总结是：
- 一种废物橙出现概率12.5% 自交出废物红
- 另一种废物橙出现概率6.25%， 自交不出红
- 最后一种橙出现概率6.25%，自交出高贵红

1,2 步做完, 此时有一堆橙1和白1，此时能量产橙2了，这时候就可以开始辨别出现的橙色是否是需要的颜色。

测交是用一种花，我们可以控制他的基因，让这一种基因和上面不确定基因的花杂交，通过后代的表现来确定前代的基因。

这里测交有概率问题，如果两种花测交的后代，能出的花色相同，只有概率不同，那肯定不行。一是记起来麻烦，二是小数据上频率并不逼近概率，算不准的。 所以区分的依据仍然是花色。

另一个问题是所有的测交手段，花色都会有一定的重叠，就是说并不是出花了就能明确区分，如果出在大家都会出的花色，那就还得等。而出了不重叠的花色，那有2种情况，一是目标基因出不了这种花色，那测交的亲本就确定不是目标基因，排除。 另一个是只有目标基因出得了这个花色，那就确定是目标基因了，确定。


#### 区分橙2
跑了下程序  
用种子红杂交的黑玫瑰和废物橙a杂交，有25%出黑，其他会出黑色，用这个可以筛掉大量废物a

如果只剩下废物2b和有用2c，那么也可可以用这个黑来筛选出对的有用橙，因为2b不会出红，而2c有50%出红，只要红了就是有用橙2c

又或者只要排除掉橙2a，因为只有他会出干扰红，如果场地够大，可以一起种橙2b和橙2c，只要自交出红色也就是目标红。
另外种子红杂交出的黑，在我的代码里是192，是纯合子，自己和自己交只能出现192（自己）
...
#### 区分红3
如果懒得区分橙色，这里还有办法区分最后的红3

但是还是建议橙2这一步就开始，因为太占地方
...
可以看出出黑这对红就废掉了，可以铲掉腾空间

更高的筛选效率有50%，没有更高了。计算下来最容易的到手的测交是种子白和种子白交配出来的紫
| 和紫杂交的后代|红|白|紫|
|---|---|---|---|
|废红3a| 50%|50%|0%|
|目标红3b| 50%|0%|50%|

出了白就铲掉，出了紫就留下来自交，蓝玫瑰近在眼前了！
